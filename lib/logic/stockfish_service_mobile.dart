...
class StockfishService {
  ...
  /// Returns a map with: best_eval (double), is_mate (bool),
  /// suggestions (List<String>), alternatives (List<double>).
  Future<Map<String, dynamic>> evaluateFenForAnalysis(
    String fen, {
    int depth = 14,
    int timeoutMs = 4000,
  }) async {
    // Try calling the remote backend analysis API first (timeout 3 seconds)
    try {
      ...
      debugPrint(
          '[StockfishService] Remote analysis failed, falling back to local Stockfish: $e');
    }
    ...
    StreamSubscription<String>? analysisSub;
    analysisSub = _engine!.stdout.listen((line) {
      if (line.startsWith('info') && line.contains(' pv ')) {
        ...
        final pvMove = pvMatch?.group(1) ?? '';

        double eval = 0.0;
        bool thisMate = false;
        if (mateMatch != null) {
          thisMate = true;
          eval = int.parse(mateMatch.group(1)!) > 0 ? 15.0 : -15.0;
        } else if (cpMatch != null) {
          eval = int.parse(cpMatch.group(1)!) / 100.0;
        }

        final isMultiPv1 =
            line.contains('multipv 1') || !line.contains('multipv');
        if (pvMove.isNotEmpty && !suggestions.contains(pvMove)) {
          suggestions.add(pvMove);
          alternatives.add(eval);
        }
        if (isMultiPv1) {
          bestEval = eval;
          isMate = thisMate;
        ...
        if (!completer.isCompleted) {
          completer.complete({
            ...
            'alternatives': alternatives.take(5).toList(),
          });
        }
        analysisSub?.cancel();
      }
    ...
    // Set MultiPV to collect top moves
    _engine!.stdin = 'setoption name MultiPV value 5';
    _engine!.stdin = 'position fen $fen';
    _engine!.stdin = 'go depth $depth movetime $timeoutMs';
    ...
    try {
      ...
      // Restore MultiPV to 1 for normal game use
